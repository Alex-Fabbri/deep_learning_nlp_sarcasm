...
if __name__ == "__main__":
    ...
    for query in test_data:
        ...
        context_sents = nltk.sent_tokenize(context.decode("utf-8"))
        out = []
        for sent in context_sents:
            words = nltk.word_tokenize(sent.encode("utf-8", 'ignore'))

            out.append(words)
        context_words.append(out)

        response_sents = nltk.sent_tokenize(response.decode("utf-8"))
        out = []
