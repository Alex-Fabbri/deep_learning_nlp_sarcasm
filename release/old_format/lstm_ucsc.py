...
def load_data(target, path, hidden_units, both, top, batch_size):
    ...
    # put into shared variables  -- only useful if using GPUa
...
def train_test_model(target, path, hidden_units, both, top, batch_size,num_epochs = 25):
    X_train, X_train_mask, y_train, X_val, X_val_mask, y_val, X_test, X_test_mask, y_test, word_idx_map,W, max_l = load_data(target, path, hidden_units, both,top, batch_size = batch_size)
    ...
    #         M: X_train_mask[index * batch_size: (index + 1) * batch_size],
    ...
    def compute_val_error(log_file=log_file, X_val=X_val,X_val_mask = X_val_mask, y_val=y_val):
        val_loss = 0.
        val_acc = 0.
        val_batches = 0
        for batch in iterate_minibatches(X_val,X_val_mask, y_val,
                                         batch_size, shuffle=False):
            x_val_mini,x_val_mini_mask,y_val_mini = batch
            v_loss, v_acc, _ = val_fn(x_val_mini,
                                      x_val_mini_mask,
                                      y_val_mini)
            val_loss += v_loss
            val_acc += v_acc
