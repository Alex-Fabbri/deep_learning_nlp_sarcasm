...
"""
def read_data_file(data_file, max_l,max_s, is_train,both=False,topSim=False):
    queries = []
    ids = []
    change_count = 0
    change_count_sentences = 0
    ...
    with open(data_file, "r") as fin:
        for line in fin:
            ...
            if len(newWords) > max_l:
                words = words[:max_l]
                change_count += 1
            if len(sentences) > max_l:
                sentences = sentences[:max_s]
                change_count_sentences +=1
            datum = {"y": int(label),
                    "text": text,
                    "num_words": len(words),
    ...
    print ("length more than %i: %i. Number of sentences more than %i: %i\n" % (max_l, change_count, max_s, change_count_sentences)) 
...
def train_test(target,vocab,w2v,input,output,w2v_file,both=False,topSim=True,w2v_type='bin'):
    ...
    train_data,train_id = read_data_file(train_file,max_l,max_s, 1,both,topSim)
    test_data,test_id = read_data_file(test_file,max_l,max_s, 0,both,topSim)
    cPickle.dump(test_data, open(output_test_file, "wb"))

    
    df = pd.DataFrame(train_data)
    max_l = np.max(df["num_words"])
    max_s = np.max(df["num_sentences"])
    print max_l
    print max_s
    ...
    cPickle.dump([train_data, W, word_idx_map, max_l, max_s], open(output_train_file, "wb"))
